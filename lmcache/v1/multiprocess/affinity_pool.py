...
class AffinityThreadPool:
    ...
    def _slot_for_key(self, affinity_key: int) -> int:
        """Return the worker slot bound to ``affinity_key``.

        Returns:
            The worker slot index for the key.
        """
        ...
        self._next_slot += 1
        logger.info(
            "AffinityThreadPool: affinity_key=%d assigned to worker slot %d "
            "of %d (thread %s); %d distinct key(s) now bound",
            affinity_key,
            slot,
            self._num_workers,
