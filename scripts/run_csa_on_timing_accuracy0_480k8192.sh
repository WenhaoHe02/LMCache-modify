...
exec 9>"${root}/run_csa_on_tokenids_no_nsys.lock"
if ! flock -n 9; then
  echo 'another CSA on token-ID harness is already running' >&2
  exit 75
fi
base_tokens=480000
...
tag="csa_on_no_nsys_accuracy0_${policy_tag}_${base_tokens}p${recompute_tokens}_$(date -u +%Y%m%d_%H%M%S)"
result_dir="${root}/results/${tag}"
container=dsv4-csa-cp8-on
mkdir -p "${result_dir}"
printf '%s\n' "${tag}" > "${root}/current_on_accuracy0_tag.txt"
printf 'starting\n' > "${result_dir}/status"
...
export LMCACHE_ABLATION_TUTTI_STARTUP_DELAY="${LMCACHE_ABLATION_TUTTI_STARTUP_DELAY:-1}"
...
grep -qx 'LMCACHE_NSYS_CAPTURE=0' "${result_dir}/process_env.txt"
grep -qx 'LMCACHE_NSYS_FULL_CAPTURE=0' "${result_dir}/process_env.txt"
grep -qx 'LMCACHE_EXEC_PREFIX=' "${result_dir}/process_env.txt"
...
ENABLE_TORCH_PROFILE=0 DISTINCT_HIT_PROMPTS=1 \
  ...
  python3 "${root}/run_hermes_trial2_480k200.py" "${STORE_WAIT_S:-60}" \
...
if ready.get("distinct_hit_prompts") is not True or len(set(hashes)) != 5:
...
if len(warmups) != 1 or len(hits) != 4:
...
times = [float(row["elapsed_s"]) for row in hits]
summary = {
    "cold_s": next(
        float(row["elapsed_s"])
        for row in rows
    ...
    "hit_s": times,
    "hit_min_s": min(times),
    "hit_median_s": statistics.median(times),
    "hit_max_s": max(times),
...
printf 'complete\n' > "${result_dir}/status"
printf '%s\n' "${tag}" > "${root}/completed_on_accuracy0_tag.txt"
trap - EXIT
cleanup
