...
tag="csa_on_no_nsys_tokenids_${base_tokens}p${recompute_tokens}_$(date -u +%Y%m%d_%H%M%S)"
result_dir="${root}/results/${tag}"
container=dsv4-csa-cp8-on
mkdir -p "${result_dir}"
printf '%s\n' "${tag}" > "${root}/current_on_tokenids_tag.txt"
printf 'starting\n' > "${result_dir}/status"
...
if log.count("attention_true_topk_profile") < 1:
...
printf 'complete\n' > "${result_dir}/status"
printf '%s\n' "${tag}" > "${root}/completed_on_tokenids_tag.txt"
trap - EXIT
cleanup
