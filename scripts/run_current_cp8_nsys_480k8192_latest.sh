...
tag="csa_on_full_nsys_accuracy0_${policy_tag}_${base_tokens}p${recompute_tokens}_$(date -u +%Y%m%d_%H%M%S)"
...
if accuracy_records != 0:
    raise SystemExit("accuracy profiling unexpectedly altered the production trace")
summary = {
    "cold_s": next(
        float(row["elapsed_s"])
        for row in rows
