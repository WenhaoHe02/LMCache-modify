...
tag="csa_off_64token_$(date -u +%Y%m%d_%H%M%S)"
result_dir="${root}/results/${tag}"
container=dsv4-csa-cp8-off
mkdir -p "${result_dir}"
printf '%s\n' "${tag}" > "${root}/current_off_64token_tag.txt"
printf 'starting\n' > "${result_dir}/status"
...
sudo docker rm -f "${container}" >/dev/null 2>&1 || true
bash "${root}/run_container_cp8_ab.sh" off > "${result_dir}/launch.log" 2>&1
sleep 15
printf 'running\n' > "${result_dir}/status"
MODE_LABEL=off BASE_TOKENS=480000 RECOMPUTE_TOKENS=8192 \
