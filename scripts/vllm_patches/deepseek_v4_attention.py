...
def _lmcache_compact_csa_gather_enabled(
    ...
    """Return whether LMCache owns this compressed CSA K cache."""
    ...
    try:
        ...
        if manager is None or not manager.owns_k_cache(compressed_k_cache):
            return False
        # Cold/recompute requests already own a complete vLLM K cache.  The
        # compact gather is required only after LMCache correction has
        # published an exact true-topK page union for the current hit.
        return manager.true_selected_blocks_for_cache(compressed_k_cache) is not None
...
def _lmcache_compact_csa_gather_plan(
    ...
    """Delegate compact plan construction to LMCache's tested helper."""
    ...
    try:
        ...
        manager = get_csa_attention_kv_prefetch_manager()
        get_selected = getattr(manager, "true_selected_blocks_for_cache", None)
        if callable(get_selected):
            selected_page_bitmap = get_selected(compressed_k_cache)
        get_prefix_coverage = getattr(
            manager,
            "true_selected_covers_cached_prefix_for_cache",
            None,
        )
        if callable(get_prefix_coverage):
            cached_prefix_fully_selected = bool(
                get_prefix_coverage(compressed_k_cache)
            )
    except (ImportError, AttributeError):
        selected_page_bitmap = None
    if cached_prefix_fully_selected:
        logger.info_once(
            "LMCache CSA true-topK covers the cached prefix; compact gather "
            "planning and remapping are bypassed"
        )
    elif selected_page_bitmap is not None:
        logger.info_once(
            "LMCache compact CSA gather reuses the correction page bitmap"
        )
    return build_compact_csa_prefill_gather_plan(
        topk_indices,
        block_table,
        compressed_seq_lens,
        query_row_offsets,
        block_size,
        selected_page_bitmap=selected_page_bitmap,
        cached_prefix_fully_selected=cached_prefix_fully_selected,
    )
...
class DeepseekV4MLAAttention(nn.Module, AttentionLayerBase):
    ...
    def _forward_prefill(
        ...
        if compact_csa_gather:
            logger.info_once(
                "LMCache compact CSA prefill gather is active: only "
                "true-topK-referenced pages are dequantized"
            )
        for chunk_idx in range(num_chunks):
            ...
            if not swa_only:
                ...
                if compact_csa_gather:
                    ...
                    if gather_block_table.device != compact_device:
                        gather_block_table = gather_block_table.to(
                            device=compact_device,
                            non_blocking=True,
                        )
                    if gather_seq_lens.device != compact_device:
                        gather_seq_lens = gather_seq_lens.to(
                            device=compact_device,
                            non_blocking=True,
                        )
                    if query_row_offsets.device != compact_device:
            ...
            # In compact mode CSA indices address the selected-page workspace;
            # the SWA region intentionally retains its original full-N offset.
