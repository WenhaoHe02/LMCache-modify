...
class LocalDiskBackend(StorageBackendInterface):
    def __init__(
        ...
        # A deterministic generation id can be reused after eviction and
        # re-admission, while its physical raw-pool extents may have changed.
        # Cache consumers therefore need a process-local publication revision
        # in addition to the content-derived generation id.
        self._csa_layout_revision_counter = 0
        self._csa_layout_revisions: dict[str, int] = {}
    ...
    def get_csa_streaming_plan_token(
        self,
        key: CacheEngineKey,
    ) -> Optional[tuple[str, int, int, int]]:
        """Return the immutable identity of a published streaming layout.

        Args:
            key: Terminal cache key whose active streaming layout is queried.

        Returns:
            ``(generation, layout_version, covered_tokens, revision)`` when a
            runtime-compatible layout is active, otherwise ``None``. The
            process-local revision changes on every publication, including a
            re-admission that reuses the deterministic generation id but owns
            different physical extents.
        """
        key_string = key.to_string()
        with self._csa_layout_lock:
            layout = self._csa_active_layouts.get(key_string)
            revision = self._csa_layout_revisions.get(key_string)
            if (
                layout is None
                or revision is None
    ...
    def _publish_csa_layout(
        ...
        with self._csa_layout_lock:
            key_string = key.to_string()
            self._csa_layout_revision_counter += 1
            self._csa_active_layouts[key_string] = layout
            self._csa_layout_revisions[key_string] = self._csa_layout_revision_counter
            self._csa_pending_layouts.pop(key_string, None)
        logger.info(
            "CSA layout published: key=%s generation=%s covered_tokens=%d objects=%d",
            key.to_string(),
            layout.generation,
            layout.covered_tokens,
            len(layout.objects),
    ...
    def get_kv_object_records(
        ...
        *,
        ...
        layout_indexes: dict[
            str,
            Optional[dict[tuple[str, int], KVObjectId]],
        ] = {}
        for index, key in enumerate(keys):
            ...
            if self._csa_streaming_layout_requested() and role in streaming_roles:
                key_string = key.to_string()
                if key_string not in layout_indexes:
                    layout = self._active_csa_layout(key)
                    if layout is None or not self._csa_layout_matches_runtime(layout):
                        layout_indexes[key_string] = None
                    else:
                        layout_indexes[key_string] = {
                            (entry.logical_role, entry.layer_id): entry.object_id
                            for entry in layout.objects
                        }
                layout_index = layout_indexes[key_string]
                object_ids.append(
                    layout_index.get((role, int(layer_id)))
    ...
    def get_kv_object_payload_lengths(
        ...
        *,
        ...
        layout_indexes: dict[
            str,
            Optional[dict[tuple[str, int], _CSAStreamingObject]],
        ] = {}
        for index, key in enumerate(keys):
            layer_id = layer_ids[index] if layer_ids is not None else 0
            role = roles[index] if roles is not None else "full"
            if self._csa_streaming_layout_requested() and role in streaming_roles:
                ...
                if key_string not in layout_indexes:
                    ...
                    if layout is None or not self._csa_layout_matches_runtime(layout):
                        ...
                        layout_indexes[key_string] = {
                            (entry.logical_role, entry.layer_id): entry
                            for entry in layout.objects
                        }
                layout_index = layout_indexes[key_string]
                if layout_index is None:
                    lengths.append(None)
                    continue
                entry = layout_index.get((role, int(layer_id)))
    ...
    def store_attention_layer_major_snapshot(
        ...
        Args:
            ...
            prefix_keys: Optional content-addressed key for every source chunk.
                ...
                composition bookkeeping. A view whose compact main is empty is
                not a self-contained request terminal: its ordinary tail state
                was intentionally omitted when it was an intermediate chunk.
    ...
    def reset_kv_object_pool_allocation(self) -> None:
        ...
        with self._csa_layout_lock:
            ...
            self._csa_layout_revisions.clear()
    ...
    def contains_streaming_terminal(
        ...
        with self.disk_lock:
            ...
            if not self._csa_layout_matches_runtime(layout):
                ...
                return False
            if not self._has_readable_csa_streaming_layout_unlocked(key):
                self._log_streaming_terminal_miss_unlocked(
                    key,
                    token_count,
                    reason="terminal_main_unavailable",
                )
                return False
            if pin:
                self.dict[key].pin()
    ...
    def remove(
        ...
        with self._csa_layout_lock:
            key_string = key.to_string()
            self._csa_active_layouts.pop(key_string, None)
            self._csa_layout_revisions.pop(key_string, None)
            self._csa_pending_layouts.pop(key_string, None)
    ...
    def _has_readable_csa_streaming_layout_unlocked(
        ...
        """Return whether a CSA layout is a self-contained request terminal.

        Intermediate chunks in a long admission intentionally have a
        metadata-only compact main because their ordinary KV groups are empty.
        They may reference valid CSA/HCA/Indexer sidecars, but cannot terminate
        a later, shorter request: the ordinary tail state for that boundary was
        never stored. Treating such a view as a hit makes lookup report tokens
        that retrieve cannot restore.
        """
        layout = self._active_csa_layout(key)
        if layout is None or not self._csa_layout_matches_runtime(layout):
            return False
        compact_entry = layout.find_object(self._csa_streaming_compact_role(), 0)
        return bool(compact_entry is not None and compact_entry.length > 0)
