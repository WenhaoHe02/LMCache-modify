...
class IndexerSSDManager:
    ...
    def __init__(
        ...
        # layer.  Keep only one post-Stage0 layer queued by default.  Tutti has
        # one SQ/CQ and one non-FIFO I/O lock per rank; queueing every Indexer
        # layer lets the single native executor repeatedly reacquire that lock
        # and starve required CSA/HCA lookahead work until its target-layer
        # gate.  A one-layer window still has the complete preceding-layer
        # compute interval in which to hide the full local Indexer read.
    ...
    def _schedule_native_indexer_through(self, position: int) -> None:
        ...
        cursor = 0
        while cursor < stop:
            target = int(self._csa_layer_ids[cursor])
            if target in self._native_indexer_scheduled_layers:
                cursor += 1
                continue
            suggest_group = getattr(
                loader,
                "suggested_deterministic_group_size",
                None,
            )
            group_size = (
                max(1, int(suggest_group(target))) if callable(suggest_group) else 1
            )
            group_end = min(len(self._csa_layer_ids), cursor + group_size)
            group = tuple(
                int(layer_id)
                for layer_id in self._csa_layer_ids[cursor:group_end]
                if int(layer_id) not in self._native_indexer_scheduled_layers
            )
            if not group:
                cursor = group_end
                continue
            fire_group = getattr(loader, "fire_deterministic_layers", None)
            if len(group) > 1 and callable(fire_group):
            ...
            for grouped_layer in group:
                self._native_indexer_scheduled_layers.add(grouped_layer)
                loader.track_layer_submission(
                    grouped_layer,
                    future,
                    request_token=request_token,
                )
            # One adaptive group intentionally extends beyond ``stop``. This
            # is the fixed-cost amortization: short layers are submitted in a
            # compact wave, while large layers report group_size == 1.
            cursor = group_end
            stop = max(stop, group_end)
    ...
    def fire_async_for_layers(
        ...
        fire_group = getattr(manager, "fire_deterministic_layers", None)
        suggest_group = getattr(
            manager,
            "suggested_deterministic_group_size",
            None,
        ...
        registered_hca = tuple(
            int(value) for value in getattr(manager, "hca_layer_ids", ())
        )
        requested = tuple(dict.fromkeys(int(value) for value in layer_ids))
        expanded: list[int] = []
        for layer_id in requested:
            if layer_id in expanded:
                continue
            group_size = (
                max(1, int(suggest_group(layer_id))) if callable(suggest_group) else 1
            )
            if layer_id in registered_hca:
                start = registered_hca.index(layer_id)
                expanded.extend(registered_hca[start : start + group_size])
            else:
                expanded.append(layer_id)
        pending_group: list[int] = []
        for layer_id in dict.fromkeys(expanded):
            with self._lock:
        ...
        if not pending_group:
            return
        if len(pending_group) > 1 and callable(fire_group):
            future = self._executor.submit(
                fire_group,
                tuple(pending_group),
    ...
    def fire_dense_csa_layers(self, layer_ids: Sequence[int]) -> None:
        ...
        fire_group = getattr(manager, "fire_deterministic_layers", None)
        suggest_group = getattr(manager, "suggested_dense_group_size", None)
        track = getattr(manager, "track_layer_submission", None)
        if not callable(fire_layer):
            return
        request_id = str(getattr(manager, "active_request_id", ""))
        request_token = getattr(manager, "active_request_token", (request_id, -1))
        registered_dense = tuple(
            int(value) for value in getattr(manager, "dense_shard_layer_ids", ())
        )
        requested = tuple(dict.fromkeys(int(value) for value in layer_ids))
        expanded: list[int] = []
        for layer_id in requested:
            group_size = (
                max(1, int(suggest_group(layer_id)))
                if callable(suggest_group)
                else 1
            )
            if group_size > 1 and layer_id in registered_dense:
                start = registered_dense.index(layer_id)
                expanded.extend(registered_dense[start : start + group_size])
            else:
                expanded.append(layer_id)
        ...
        if not pending_group:
            return
        if len(pending_group) > 1 and callable(fire_group):
            future = self._dense_shard_executor.submit(
                fire_group,
                tuple(pending_group),
                label="csa_dense_group",
                request_token=request_token,
            )
        else:

            def _fire(
                target_layer_id: int = pending_group[0],
    ...
    def wait_for_csa_attention_kv_prediction(self, layer_id: int) -> bool:
        """Join all submitted prediction work at the target gate.

        The cache-hit prefill path fires the target CSA layer's HC-proxy top-K
        from the previous decoder layer's FFN/MoE window. Once submitted, both
        queued and running work must finish before collective preflight. In
        particular, marking a layer expired before joining its producer makes
        the producer discard an otherwise valid result and causes a one-rank
        ``LOCAL_DIRECT`` fallback on the first request of a new shape.

        A proxy-completion future can enqueue a child I/O future immediately
        before it returns. The join therefore repeats under ``_lock`` until the
        per-layer future chain is stable.

        Args:
            layer_id: CSA layer whose async proxy prediction should be joined.

        Returns:
            ``True`` when every submitted prediction completed, or ``False``
            when no prediction was submitted or work was externally cancelled.
        ...
        while pending:
            ...
            # A running proxy-completion future submits the actual I/O future
            # before it returns. The first snapshot cannot contain that child,
            # so taking it only once lets collective preflight race the child
            # on one rank. Once every joined producer has returned, any child
            # it can create is visible under ``_lock``; repeat until stable.
            with self._lock:
                pending = [
                    future
                    for future in self._proxy_futures.get(int(layer_id), ())
                    if future not in joined
                ]
        if cancelled:
            with self._lock:
                self._expired_proxy_layers.add(int(layer_id))
        if _profile_accuracy_enabled():
            logger.info(
                "IndexerSSDManager: prediction_target_gate layer %d "
                "submitted=1 was_ready=%d cancelled=%d running=%d joined=%d "
                "wait_ms=%.3f",
                layer_id,
                int(ready),
                int(cancelled),
