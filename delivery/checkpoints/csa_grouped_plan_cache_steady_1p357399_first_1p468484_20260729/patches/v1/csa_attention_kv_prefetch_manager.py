...
def _static_io_group_setting(name: str, default: int) -> int:
    """Return a non-negative integer setting for static layer I/O grouping."""
    try:
...
@dataclass(slots=True)
class _PreparedShardGather:
    """Rank-local SSD phase awaiting model-thread collective completion."""
    ...
    local_event: Optional[torch.cuda.Event]
    local_objects: List[Any]
    local_completed: torch.Tensor
    local_error: Optional[Exception]
    io_priority: str
    io_range: Optional[CsaNvtxRange]
    operation_id: str
    collective_submitted: bool = False
    ...
    * :meth:`patch_indexer_forward` — monkey-patch one CSA Indexer
    ...
    def __init__(
        ...
        *,
        ...
        self._prepared_shard_condition = threading.Condition()
        self._prepared_shard_gathers: Dict[int, _PreparedShardGather] = {}
        self._preparing_shard_layers: set[int] = set()
    ...
    def _deactivate_request_locked(self, timeout_s: float = 30.0) -> bool:
        ...
        if not self._drain_exact_topk_futures(deadline):
            ...
            return False
        if not self._discard_prepared_shard_gathers(deadline):
            logger.error(
                "CSAAttentionKVPrefetchManager: prepared shard work could not "
                "be discarded safely"
            )
            return False
    ...
    def fire_deterministic_layer(
        ...
        *,
        ...
        io_priority = (
            "lookahead"
            if label
            in {"indexer_native_stream", "hca_deterministic", "csa_dense_group"}
            else "demand"
        )
        self._submit_reads(
            int(layer_id),
    ...
    def suggested_deterministic_group_size(self, layer_id: int) -> int:
        """Return an adaptive group size for a full-layer static read.

        The target is byte based rather than context-length based, so Indexer
        K and HCA automatically choose different group sizes for the same
        prompt. Large layers remain one-at-a-time to preserve pipeline
        granularity; small layers amortize Tutti prepare/submit/poll overhead.

        Args:
            layer_id: Any registered deterministic Indexer or HCA layer.

        Returns:
            A value in ``[1, LMCACHE_DSV4_STATIC_IO_GROUP_MAX]``. Returns one
            when the fused raw-scatter capability or a complete layer-major
            plan is unavailable.
        """
        max_group = max(
            1,
            _static_io_group_setting("LMCACHE_DSV4_STATIC_IO_GROUP_MAX", 8),
        )
        target_mib = max(
            1,
            _static_io_group_setting(
                "LMCACHE_DSV4_STATIC_IO_GROUP_TARGET_MIB",
                32,
            ),
        )
        state = self._layers.get(int(layer_id))
        scatter = (
            getattr(_csa_c_ops, "scatter_rows_from_object_ptrs", None)
            if _csa_c_ops is not None
            else None
        )
        if (
            max_group <= 1
            or not callable(scatter)
            or state is None
            or not state.chunks
            or state.layer_major_dst_rows_table is None
            or any(not chunk.layer_major for chunk in state.chunks)
        ):
            return 1
        block_nbytes = int(state.compressed_block_size * state.token_bytes)
        layer_nbytes = int(state.chunks[-1].end_compressed_block) * block_nbytes
        if layer_nbytes <= 0:
            return 1
        target_nbytes = target_mib * 1024**2
        return min(max_group, max(1, target_nbytes // layer_nbytes))

    def suggested_dense_group_size(self, layer_id: int) -> int:
        """Return a safe local grouping size for an early dense CSA layer.

        Dense layers use the collective path whenever its static cost model is
        favorable. Only the local-direct side of that decision is eligible for
        fused multi-layer I/O. This preserves fine-grained overlap for larger
        contexts while amortizing fixed submit cost for small ones.

        Args:
            layer_id: Registered dense CSA transformer layer id.

        Returns:
            One for a shard-gather candidate, otherwise the byte-adaptive
            deterministic group size.
        """
        state = self._layers.get(int(layer_id))
        if state is None or not state.chunks:
            return 1
        union_blocks = int(state.chunks[-1].end_compressed_block)
        sharding_enabled = bool(
            self._data_group == "csa"
            and self._shard_transport is not None
            and self._shard_config.enabled
            and self._shard_config.csa_enabled
            and int(layer_id) in self._shard_config.dense_layers
        )
        if (
            sharding_enabled
            and union_blocks >= int(self._shard_config.min_union_blocks)
            and self._static_shard_cost_preferred(union_blocks)
        ):
            return 1
        return self.suggested_deterministic_group_size(int(layer_id))

    def fire_deterministic_layers(
        self,
        ...
        *,
        ...
        """Submit several complete static layers in one Tutti call.

        Only full layer-major, non-resident plans use the grouped path. Any
        incompatible preflight falls back to the proven per-layer method
        before I/O starts; an error after submission is propagated because a
        partially written K cache must never be retried speculatively.

        Args:
            layer_ids: Ordered deterministic Indexer or HCA layer ids.
            label: Profiling label and Tutti admission class selector.
            request_token: Request generation captured when work was queued.

        Returns:
            ``True`` when every requested layer was materialized.

        Raises:
            RuntimeError: If grouped I/O starts but does not complete fully.
        """
        ordered_ids = tuple(dict.fromkeys(int(value) for value in layer_ids))
        if not ordered_ids:
            return False
        if len(ordered_ids) == 1:
            return self.fire_deterministic_layer(
                ordered_ids[0],
                label=label,
                request_token=request_token,
            )
        ...
        try:
            grouped = self._submit_deterministic_layers_active(
                ordered_ids,
                label=label,
            )
        finally:
    ...
    def wait_for_tracked_submission(
        ...
        while True:
            with self._scheduled_layer_futures_lock:
                future = self._scheduled_layer_futures.get(target_layer)
            if future is None:
                return True
            ...
            try:
                ...
                logger.error(
                    ...
                    "chain timed out and remains tracked for request cleanup",
                    layer_id,
                )
                return False
            except Exception:
                with self._request_state:
                    self._request_cleanup_failed = True
                logger.exception(
                    "CSAAttentionKVPrefetchManager: tracked layer %d submission failed",
                    layer_id,
                )
                with self._scheduled_layer_futures_lock:
                    if self._scheduled_layer_futures.get(target_layer) is future:
                        self._scheduled_layer_futures.pop(target_layer, None)
                return False

            # A proxy producer may replace itself with the rank-local SSD read
            # future before it completes. Joining only the captured outer
            # future leaves a narrow window where collective preflight observes
            # no prepared descriptor on one rank. Re-read under the same lock
            # and continue until the tracked future is both complete and stable.
            with self._scheduled_layer_futures_lock:
                current = self._scheduled_layer_futures.get(target_layer)
    ...
    def wait_for_layer(self, layer_id: int, timeout_s: float = 2.0) -> bool:
        ...
        deadline = time.monotonic() + max(0.0, timeout_s)
        with self._scheduled_layer_futures_lock:
            scheduled = self._scheduled_layer_futures.get(int(layer_id))
        if scheduled is not None:
            try:
                ...
                logger.error(
                    ...
                    "submission timed out and remains tracked for cleanup",
                    layer_id,
                )
                return False
            except Exception:
                ...
                if bool(getattr(scheduled, "done", lambda: False)()):
                    with self._scheduled_layer_futures_lock:
                        if (
                            self._scheduled_layer_futures.get(int(layer_id))
                            is scheduled
                        ):
                            self._scheduled_layer_futures.pop(int(layer_id), None)
        state = self._layers.get(int(layer_id))
        if state is None:
            return True
        with state.pending_reads_lock:
        ...
        with self._patch_lock:
            ...
            def _patched_forward(*args: Any, **kwargs: Any) -> torch.Tensor:
                ...
                try:
                    ...
                    # The residual proxy can enqueue the rank-local shard read
                    # while ``waiter`` is running. The earlier join therefore
                    # cannot observe that newly tracked producer. Join again
                    # after the proxy completes so every rank publishes its
                    # prepared descriptor before model-thread consensus.
                    if not mgr.wait_for_tracked_submission(layer_id):
                        raise RuntimeError(
                            f"tracked shard submission failed for layer {layer_id}"
                        )
                    if not mgr.complete_prepared_shard_gather(layer_id):
                        raise RuntimeError(
                            f"prepared shard gather is unavailable for layer {layer_id}"
                        )
    ...
    def _submit_deterministic_layers_active(
        self,
        layer_ids: Sequence[int],
        *,
        label: str,
    ) -> bool:
        """Materialize compatible full-layer plans as one active submission."""
        states: list[CSAAttentionKVLayerState] = []
        covered_by_state: list[int] = []
        for layer_id in layer_ids:
            ...
            if covered_end <= 0:
                return False
            with state.pending_reads_lock:
                if state.pending_read_count or bool(
                    torch.any(state.resident_blocks_bitmap[:covered_end])
                ):
                    return False
            states.append(state)
            covered_by_state.append(covered_end)

        pending_ids_by_state: list[torch.Tensor] = []
        io_ranges: list[tuple[Optional[CsaNvtxRange], str]] = []
        for state, covered_end in zip(states, covered_by_state, strict=True):
            pending_ids = torch.arange(covered_end, dtype=torch.int64)
            with state.pending_reads_lock, torch.inference_mode():
                state.pending_reads_bitmap[pending_ids] = True
                state.pending_read_count += covered_end
            pending_ids_by_state.append(pending_ids)
            operation_id = f"{label}_group-{time.monotonic_ns()}-{state.layer_id}"
            io_range = csa_pipeline_nvtx.start_io(
                layer_id=int(state.layer_id),
                target_layer_id=int(state.layer_id),
                operation_id=operation_id,
                request_id=self.active_request_id,
                attributes={
                    "kind": label,
                    "blocks": covered_end,
                    "group_layers": len(states),
                },
            )
            io_ranges.append((io_range, operation_id))

        try:
            device = states[0].k_cache_tensor.device
            device_guard = (
                torch.cuda.device(device)
                if states[0].k_cache_tensor.is_cuda
                else nullcontext()
            )
            stream_guard = (
                torch.cuda.stream(self._prepare_stream_for(device))
                if states[0].k_cache_tensor.is_cuda
                else nullcontext()
            )
            with torch.inference_mode(), device_guard, stream_guard:
                self._issue_full_layer_major_reads_batch(
                    states,
                    io_priority=(
                        "lookahead"
                        if label in {"indexer_native_stream", "hca_deterministic"}
                        else "demand"
                    ),
                )
        except Exception as exc:
            for state, pending_ids, (io_range, operation_id) in zip(
                states,
                pending_ids_by_state,
                io_ranges,
                strict=True,
            ):
                csa_pipeline_nvtx.finish_io(
            ...
            raise RuntimeError(
                f"failed grouped deterministic I/O for layers {tuple(layer_ids)}"
            ) from exc
        ...
        for state, pending_ids, (io_range, operation_id) in zip(
            ...
            with state.pending_reads_lock, torch.inference_mode():
                state.resident_blocks_bitmap[pending_ids] = True
                state.pending_reads_bitmap[pending_ids] = False
                state.pending_read_count = max(
                    0,
                    state.pending_read_count - int(pending_ids.numel()),
                ...
                state.last_drain_event = None
                state.pending_drains.append((None, [], io_range, operation_id))
                state.pending_reads_lock.notify_all()
        return True

    def _issue_full_layer_major_reads_batch(
        self,
        states: Sequence[CSAAttentionKVLayerState],
        *,
        io_priority: str,
    ) -> None:
        """Read and scatter several complete layer-major plans in one call."""
        scatter = (
            getattr(_csa_c_ops, "scatter_rows_from_object_ptrs", None)
            if _csa_c_ops is not None
            else None
        )
        if not states or not callable(scatter):
            raise RuntimeError("grouped full-layer raw scatter is unavailable")
        first_device = states[0].k_cache_tensor.device
        if any(state.k_cache_tensor.device != first_device for state in states):
            raise RuntimeError("grouped layers span multiple CUDA devices")

        contexts: list[tuple[Any, ...]] = []
        entries: list[tuple[int, CSAAttentionKVChunkLoc, int, int]] = []
        keys: list[CacheEngineKey] = []
        disk_metas: list[Optional[DiskCacheMetadata]] = []
        read_ranges: list[tuple[KVObjectByteRange, ...]] = []
        total_nbytes = 0
        for context_index, state in enumerate(states):
            dst_rows_table = state.layer_major_dst_rows_table
            if dst_rows_table is None:
                raise RuntimeError("grouped layer is missing destination rows")
            block_nbytes = int(state.compressed_block_size * state.token_bytes)
            if block_nbytes <= 0:
                raise RuntimeError("grouped layer has an invalid block size")
            ...
            contexts.append((state, dst_rows_table, block_nbytes, k_cache))
            expected = 0
            for chunk in state.chunks:
                if not chunk.layer_major or chunk.first_compressed_block != expected:
                    raise RuntimeError("grouped layer-major plan has a gap")
                ...
                if read_nbytes < int(chunk.payload_skip) + payload_nbytes:
                    raise RuntimeError("grouped layer-major read is truncated")
                entries.append(
                    (
                        context_index,
                        chunk,
                ...
                total_nbytes += read_nbytes
                expected = chunk.end_compressed_block

        scatter_stream = self._scatter_stream_for(first_device)
        completed_entries: set[int] = set()
        ...
        def _scatter_raw_batch(
            ...
            with torch.inference_mode(), torch.cuda.stream(scatter_stream):
                for local_index, offset, nbytes in zip(
                    ...
                    entry_index = batch_start + int(local_index)
                    context_index, chunk, payload_skip, payload_nbytes = entries[
                        entry_index
                    ]
                    if int(nbytes) < payload_skip + payload_nbytes:
                        raise RuntimeError("grouped raw layer read is truncated")
                    state, dst_rows, block_nbytes, k_cache = contexts[context_index]
                    ...
                    completed_entries.add(entry_index)
        ...
        self._tutti_loader.load_chunks_to_hbm(
            ...
            on_raw_batch_loaded=_scatter_raw_batch,
        )
        if len(completed_entries) != len(entries):
            raise RuntimeError(
                f"grouped layer read completed {len(completed_entries)}/"
                f"{len(entries)} objects"
            )
        with torch.inference_mode(), torch.cuda.stream(scatter_stream):
            for state in states:
                covered_end = int(state.chunks[-1].end_compressed_block)
                state.in_pool_bitmap[:covered_end].fill_(True)
        scatter_stream.synchronize()
        if _timing_enabled():
            logger.info(
                "TUTTI_PROFILE static_layer_group layers=%s objects=%d bytes=%d",
                [int(state.layer_id) for state in states],
                len(entries),
                total_nbytes,
            )
        ...
        *,
        ...
        active_sharding = bool(
            ...
            and (
                label == "dense"
                or (
                    label.startswith("predicted_")
                    and int(layer_id) not in self._shard_config.dense_layers
                )
            )
        )
        ...
        if candidate_ids.numel() == 0:
            return True
        shard_union_blocks = int(candidate_ids.numel())
        shard_bypass_reason: Optional[str] = None
        if active_sharding:
            if shard_union_blocks < int(self._shard_config.min_union_blocks):
                shard_bypass_reason = "below_minimum"
            elif not self._static_shard_cost_preferred(shard_union_blocks):
                shard_bypass_reason = "static_cost"
        if active_sharding and shard_bypass_reason is None:
            preferred_mode = (
                SSDReadMode.SHARD_GATHER_DENSE
                if label == "dense"
                else SSDReadMode.SHARD_GATHER_PREDICTED
            )
            return self._prepare_sharded_reads_active(
                state,
                candidate_ids,
                label=label,
                io_priority=io_priority,
        ...
        if shard_bypass_reason is not None and _timing_enabled():
            logger.info(
                "CSAAttentionKVPrefetchManager: shard bypass layer=%d "
                "label=%s union_blocks=%d min_union_blocks=%d reason=%s; "
                "submitting early LOCAL_DIRECT",
                layer_id,
                label,
                shard_union_blocks,
                int(self._shard_config.min_union_blocks),
                shard_bypass_reason,
            )
        with state.pending_reads_lock:
        ...
        try:
            ...
            stream_guard = (
                torch.cuda.stream(self._prepare_stream_for(k_cache_tensor.device))
                if io_priority == "lookahead"
                and isinstance(k_cache_tensor, torch.Tensor)
    ...
    def complete_prepared_shard_gather(self, layer_id: int) -> bool:
        """Complete one prepared CSA shard gather on the model thread.

        Background workers perform only rank-local SSD reads.  This method is
        called from the target layer's Indexer hook, which every rank reaches
        in model order, and is the only request-time path allowed to launch
        collectives on the dedicated CSA process group.

        Args:
            layer_id: Target CSA transformer layer id.

        Returns:
            ``True`` when no gather was prepared or the complete union was
            materialized safely.

        Raises:
            RuntimeError: If consensus, fallback I/O, or data gathering fails.
        """
        with self._request_state:
            if self._request_lifecycle != "active":
                return False
            self._active_submissions += 1
        try:
            with self._prepared_shard_condition:
                prepared = self._prepared_shard_gathers.get(int(layer_id))
            if prepared is None:
                state = self._layers.get(int(layer_id))
                transport = self._shard_transport
                if (
                    ...
                    or not self._shard_config.csa_enabled
                    or int(layer_id) in self._shard_config.disabled_layers
                ):
                    return True
                mode = (
                    SSDReadMode.SHARD_GATHER_DENSE
                    if int(layer_id) in self._shard_config.dense_layers
                    else SSDReadMode.SHARD_GATHER_PREDICTED
                )
                descriptor = CollectiveDescriptor(
                    ...
                    mode=mode,
                    partition=partition_block_union([], transport.world_size),
                )
                device = state.k_cache_tensor.device
                device_guard = (
                    torch.cuda.device(device)
                    if state.k_cache_tensor.is_cuda
                    else nullcontext()
                )
                with (
                    torch.inference_mode(),
                    device_guard,
                    self._shard_collective_lock,
                ):
                    if transport.preflight(
                        ...
                        raise RuntimeError(
                            "empty shard sentinel unexpectedly passed consensus"
                        )
                return True
            ...
            device_guard = (
                torch.cuda.device(device)
                if isinstance(state.k_cache_tensor, torch.Tensor)
            ...
            try:
                with (
                    ...
                    event, memory_objects, completed_ids = (
                        self._complete_prepared_shard_gather(prepared)
                    )
            except Exception:
                with self._request_state:
                    self._request_cleanup_failed = True
                logger.exception(
                    "CSAAttentionKVPrefetchManager: model-thread shard "
                    "completion failed for layer %d",
                    layer_id,
                )
                raise
            ...
            if not torch.equal(completed_ids, prepared.selected):
                with self._request_state:
                    self._request_cleanup_failed = True
                raise RuntimeError(
                    f"shard completion materialized {int(completed_ids.numel())}/"
                    f"{int(prepared.selected.numel())} selected blocks for "
                    f"layer {layer_id}"
                )
            if event is not None and state.k_cache_tensor.is_cuda:
            ...
            with state.pending_reads_lock:
                with torch.inference_mode():
                    state.resident_blocks_bitmap[completed_ids] = True
                    state.pending_reads_bitmap[prepared.new_ids] = False
                state.pending_read_count = max(
                    0,
                    state.pending_read_count - int(prepared.new_ids.numel()),
                )
                state.last_drain_event = event
                state.pending_drains.append(
                    (
                ...
                state.pending_reads_lock.notify_all()
            with self._prepared_shard_condition:
                if self._prepared_shard_gathers.get(int(layer_id)) is prepared:
                    self._prepared_shard_gathers.pop(int(layer_id), None)
            return True
        finally:
            with self._request_state:
                self._active_submissions -= 1
    ...
    def _prepare_sharded_reads_active(
        self,
        state: CSAAttentionKVLayerState,
        selected: torch.Tensor,
        *,
        ...
        """Read only this rank's owner rows without launching collectives."""
        transport = self._shard_transport
        if transport is None:
            raise RuntimeError("CSA shard transport is unavailable")
        layer_id = int(state.layer_id)
        selected = torch.unique(
            selected.to(device="cpu", dtype=torch.int64),
            sorted=True,
        )
        with self._prepared_shard_condition:
            while layer_id in self._preparing_shard_layers:
                self._prepared_shard_condition.wait()
            existing = self._prepared_shard_gathers.get(layer_id)
            if existing is not None:
                existing_ids = set(int(value) for value in existing.selected.tolist())
                requested_ids = set(int(value) for value in selected.tolist())
                if requested_ids.issubset(existing_ids):
                    return True
                raise RuntimeError(
                    f"layer {layer_id} already has a different prepared shard union"
                )
            self._preparing_shard_layers.add(layer_id)

        new_ids = torch.empty(0, dtype=torch.int64)
        operation_id = f"{label}-{time.monotonic_ns()}"
        ...
        try:
            ...
            io_range = csa_pipeline_nvtx.start_io(
                ...
                operation_id=operation_id,
                request_id=self.active_request_id,
                attributes={"kind": label, "blocks": int(new_ids.numel())},
            )
            ...
            if local_capability:
                try:
                    ...
                    # Generic Tutti loads preserve a dependency on the
                    # caller's current stream. Make that dependency the
                    # manager's preparation stream, never vLLM's model/default
                    # stream, so the rank-local SSD worker cannot wait on a TP
                    # or MoE collective while holding Tutti's queue lock.
                    with torch.inference_mode(), device_guard, stream_guard:
                    ...
                    local_error = exc
                    logger.exception(
                        "CSAAttentionKVPrefetchManager: owner SSD phase failed "
                        "layer=%d owned=%d; model-thread gate will fail closed",
                        layer_id,
                        int(owned.numel()),
                    )
            prepared = _PreparedShardGather(
                state=state,
            ...
            with self._prepared_shard_condition:
                self._prepared_shard_gathers[layer_id] = prepared
            logger.info(
                "CSA_SHARD_LOCAL_READY layer=%d mode=%s union_blocks=%d "
                "owned_blocks=%d local_ok=%d",
                layer_id,
                mode.value,
                int(selected.numel()),
                int(owned.numel()),
                int(local_error is None and local_capability),
            )
            return True
        except Exception:
            ...
            with self._prepared_shard_condition:
                self._preparing_shard_layers.discard(layer_id)
                self._prepared_shard_condition.notify_all()

    def _complete_prepared_shard_gather(
        self,
        prepared: _PreparedShardGather,
    ) -> Tuple[Optional[torch.cuda.Event], List[Any], torch.Tensor]:
        """Run consensus and data gathering for a rank-local prepared read."""
        transport = self._shard_transport
        if transport is None:
            raise RuntimeError("CSA shard transport disappeared before completion")
        descriptor = prepared.descriptor
        state = prepared.state
        device = state.k_cache_tensor.device
        owned = torch.as_tensor(
            descriptor.partition.blocks_for_rank(transport.rank),
            dtype=torch.int64,
            device="cpu",
        )
        local_complete = bool(
            prepared.local_error is None
            and prepared.local_capability
            and torch.equal(
                torch.unique(prepared.local_completed.cpu(), sorted=True),
                owned,
            )
        )
        # Owner I/O is already complete when the model thread reaches this
        # gate, so one metadata+readiness consensus is sufficient.  Keeping a
        # single control collective reduces target-gate latency and guarantees
        # the same collective count for real tickets and empty sentinels.
        ...
        if not agreed:
            logger.warning(
                "CSAAttentionKVPrefetchManager: model-thread shard consensus "
                "rejected layer=%d; using target-gate LOCAL_DIRECT",
                state.layer_id,
            )
            fallback_event, fallback_objects, fallback_completed = (
                self._issue_local_reads(
                    state,
                    prepared.new_ids,
                    io_priority="demand",
            ...
            if not torch.equal(
                torch.unique(fallback_completed.cpu(), sorted=True),
                prepared.new_ids,
            ):
                raise RuntimeError(
                    f"target-gate fallback did not complete layer {state.layer_id}"
                )
            return (
                fallback_event or prepared.local_event,
                [*prepared.local_objects, *fallback_objects],
                prepared.selected,
            )
        logical_rows = self._logical_destination_rows(state)
        if logical_rows is None or state.block_slot_scatter:
            raise RuntimeError("prepared CSA shard gather lost its destination layout")
        k_cache_rows = state.k_cache_tensor.view(torch.uint8).reshape(
            int(state.k_cache_tensor.shape[0]),
            -1,
        )
        prepared.collective_submitted = True
        gather_event = transport.gather_into(
            ...
            local_ready_event=prepared.local_event,
        ...
        logger.info(
            ...
            "gather_total_bytes=%d union_hash=%d sequence=%d thread=model",
            state.layer_id,
        ...
        return gather_event, prepared.local_objects, prepared.selected

    def _discard_prepared_shard_gathers(self, deadline: float) -> bool:
        """Drain rank-local prepared reads that never reached their gate."""
        with self._prepared_shard_condition:
            if self._preparing_shard_layers:
                return False
            prepared_items = tuple(self._prepared_shard_gathers.items())
            if any(item.collective_submitted for _, item in prepared_items):
                # A failed data collective can still own CUDA buffers or write
                # destination rows. Reusing this worker would permit a late
                # cross-request write; require process teardown instead.
                return False
        for layer_id, prepared in prepared_items:
            event = prepared.local_event
            if event is not None:
                query = getattr(event, "query", None)
                if not callable(query):
                    return False
            ...
            for memory_object in prepared.local_objects:
                ref_count_down = getattr(memory_object, "ref_count_down", None)
                if callable(ref_count_down):
                    ref_count_down()
            state = prepared.state
            with state.pending_reads_lock:
                ...
                state.pending_reads_lock.notify_all()
            with self._prepared_shard_condition:
                if self._prepared_shard_gathers.get(layer_id) is prepared:
                    self._prepared_shard_gathers.pop(layer_id, None)
        return True
    ...
    def _issue_reads(
        ...
        *,
        ...
        """Issue a rank-local read; collectives use the model-thread gate."""
        if preferred_mode is not None:
            raise RuntimeError(
                "shard-gather collectives must be prepared with "
                "_prepare_sharded_reads_active and completed on the model thread"
            )
        return self._issue_local_reads(
            state,
            local_fallback_ids if local_fallback_ids is not None else sorted_block_ids,
            io_priority=io_priority,
        )
        ...
        *,
        ...
        segment_read_ranges = [
            compile_layer_major_read_ranges(
                ...
                layer_byte_offset=int(chunk.layer_byte_offset),
                aligned_read_length=int(chunk.read_length),
            )
            for segment in segments
        ]
        if any(
            byte_range.offset % 512 or byte_range.length % 512
            for ranges in segment_read_ranges
            for byte_range in ranges
        ):
            raise RuntimeError(
                "layer-major Tutti plan contains a non-sector-aligned range"
            )
        load_kwargs: Dict[str, Any] = {
            "shapes_per_key": None,
            "file_offsets": [0] * len(segments),
            "read_ranges_per_key": segment_read_ranges,
            "io_priority": io_priority,
            "before_batch": _restore_lba_cache,
            "max_batch_ios": 256,
            "max_batch_bytes": 128 * 1024**2,
    ...
    def _static_shard_cost_preferred(self, union_blocks: int) -> bool:
        """Return the rank-invariant static shard cost decision.

        This check intentionally excludes local health and layout state.  It
        is therefore safe for every rank to bypass the collective before any
        owner I/O is prepared.  Rank-local capability is still checked by
        :meth:`_shard_capability_for` once the static model selects sharding.

        Args:
            union_blocks: Number of unique CSA blocks requested by the layer.

        Returns:
            ``True`` when the configured static model clears the normal
            hysteresis margin for a shard-gather operation.
        """
        transport = self._shard_transport
        if transport is None or union_blocks <= 0:
            return False
        estimate = self._shard_decisions.estimate(
            int(union_blocks),
            self._bytes_per_block,
            transport.world_size,
        )
        return bool(estimate.benefit_ms > self._shard_config.hysteresis_ms)
    ...
    def _indexer_cp_owned_blocks(
        ...
        """Return the complete native Indexer-K block range on every rank.

        The current native Indexer kernel consumes a complete local K cache.
        Reading only CP-owned rows without a matching gather leaves the cache
        incomplete and is therefore never a valid execution mode.  Keep this
        invariant here as a runtime backstop in addition to configuration
        validation.
        """
        config = self._shard_config
        if config.indexer_enabled or config.indexer_cp_verified:
            reason = "native_indexer_requires_complete_local_k_cache"
            key = (-1, reason)
            if key not in self._cp_fallback_reasons:
                self._cp_fallback_reasons.add(key)
                logger.error(
                    "INDEXER_CP_LOCAL_DISABLED reason=%s active_mode=%s",
                    reason,
